...
do
  ...
  uapi --output=jsonpretty --user=${user} Tokens create_full_access name='TNC-WP-Toolbox-${user}' | jq -r '.result.data.token' > /home/${user}/.tnc/cp-api-key
  chown ${user}:${user} -R /home/${user}/.tnc/
  chmod 0600 /home/${user}/.tnc/cp-*
done
